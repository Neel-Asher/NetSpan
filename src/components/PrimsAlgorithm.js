export function initializePrimsAlgorithm(nodes, edges) {
    if (nodes.length === 0) {
        return {
            step: 0,
            totalSteps: 0,
...
export function executePrimStep(state, nodes, edges) {
    ...
    const newMstEdges = [...state.mstEdges, { ...selectedEdge, status: 'accepted' }];
    ...
    const newEdges = edges
        .filter(edge => (edge.source === newNode || edge.target === newNode) &&
...
export function getPrimEdgesForVisualization(state, allEdges) {
    return allEdges.map(edge => {
        ...
        if (state.currentEdge && state.currentEdge.id === edge.id) {
            return { ...edge, status: 'considering' };
        }
